...
@gin.configurable
class ScalableNatureDQNNetworkWithOneExtraFFN(nn.Module):
  """The convolutional network used to compute the agent's Q-values."""
...
@gin.configurable
class NatureDQNNetworkWithOneExtraFFNandScalablePNLayer(nn.Module):
  """The convolutional network used to compute the agent's Q-values."""
...
class ScalableNatureDQNNetworkWithDoubleWidth(nn.Module):
  """The convolutional network used to compute the agent's Q-values."""
  ...
  def __call__(self, x):
    ...
    x = x.reshape((-1))  # flatten
    layer = nn.Dense(features=_scale_width(1024), kernel_init=initializer)
    x = layer(x)
    x = nn.relu(x)
    x = self._record_activations(x, layer)
    layer = nn.Dense(
...
class NatureDQNNetworkScaledPenultimate(nn.Module):
  """The convolutional network used to compute the agent's Q-values."""
